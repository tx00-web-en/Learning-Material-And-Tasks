...
2. **Clone the Sample Solution**:
   Instead of starting from scratch, we will start from the tours API from last week. You can either use the code that you wrote or this code. If using the sample solution:
...
2. **Add `morgan` to `app.js`**:
   ...
   - **Explanation**:
     ...
     - **tiny**: Minimal output.
   - You can also create custom formats (e.g., `app.use(morgan(':method :url :status :res[content-length] - :response-time ms'));`).
...
To protect routes, implement custom middleware.
