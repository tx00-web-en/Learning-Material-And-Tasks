...
In Lab 1 you learned the fundamentals of API testing with Vitest and Supertest.  
...
| `afterAll` closes the connection | Same reason as Lab 1 – prevents Vitest from hanging after tests complete. |
...
Target output shape in the Vitest reporter:
...
Vitest can report how much of your source code is exercised by the tests. Add the `--coverage` flag to generate a report:

```bash
npm test -- --coverage
```

Vitest will output a table showing the percentage of **statements**, **branches**, **functions**, and **lines** covered for each source file, and generate an HTML report inside `coverage/lcov-report/`.
...
> **Note:** To use Vitest coverage with `--coverage`, install the `@vitest/coverage-v8` dependency first.

---

### 5.6 Vitest quick reference
...
npm test  --  --testNamePattern="..."
         ↑           ↑
    npm stops here  passed straight to Vitest
```
...
- [Skip / only tests in Jest/Vitest](https://codewithhugo.com/run-skip-single-jest-test/)
